...
//                                         resultAddress = userBookingBikeInfo.getAddress();
                                         ...
                                         locationLongitude = Double.valueOf(bookingBikeInfo.getLongitude());
                                         locationLatitude = Double.valueOf(bookingBikeInfo.getLatitude());
...
//            this.bikeInfo = bikeInfo;
    ...
    private void ringDown(String result, String time) {
        ...
        OkHttpUtils.post().url(Api.BASE_URL + Api.FINDBIKERING).params(map).build().execute(new StringCallback() {
            @Override
            public void onError(Call call, Exception e, int id) {
                ToastUtils.showLong(MainActivity.this, "服务器正忙");
            }
                ...
                if (JsonUtils.isSuccess(response)) {
                    try {
                        ...
                        if (Integer.valueOf(count) < 5) {
                            ...
                            StyledDialog.buildMdLoading(MainActivity.this, "正在预定..", false, false).setMsgColor(R.color.color_ff).show();
...
//            }
    ...
    private void receiveFromManual(CodeEvent info) {
        ...
        if (info != null) {
            ...
            if (uID != null && phone != null && result != null && mToken != null) {
                openScan(uID, phone, result, mToken);
            }
        }
    }
...
//        //先清除图层
//        mMap.clear();
//        BitmapDescriptor dragLocationIcon = BitmapDescriptorFactory.fromResource(R.mipmap.search_center_ic);
